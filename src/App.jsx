...
function App() {
  ...
  const [listaGiochi, setListaGiochi] = useState([]);
   ...
   useEffect(() => {
    const fetchGiochi = async () => { //Credo recuperi asincronamente come funzione i giochi
      try { //Prova a fare la richiesta in modo chec cosi possiamo sapere se non va bene

        ///RICHIESTA POST


        const response = await fetch('http://localhost:5000/api/getgiochi', { titolo: "" }); //Effettua una richiesta GET all'API
